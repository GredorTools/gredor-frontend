...
import { usePrepopulateSection } from "../composables/usePrepopulateSection";
  ...
  belopprader.value.reduce(
    (result: Belopprad[][], belopprad: Belopprad) => {
      ...
      for (let i = 0; i < groups.length; i++) {
        if (isBeloppradInTaxonomyItemList([groups[i], ...groups[i].childrenFlat], belopprad)) {
          groupIndex = i;
          break;
        }
      }
    ...
    <div class="accordion-item" v-for="(group, groupIndex) in groups" :key="groupIndex">
      <div class="accordion-header">
        <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
          :data-bs-target="`#forvaltningsberattelse-accordion${groupIndex}`" aria-expanded="true"
          :aria-controls="`forvaltningsberattelse-accordion${groupIndex}`">
          {{ group.additionalData.displayLabel }}
        </button>
      </div>
      <div :id="`forvaltningsberattelse-accordion${groupIndex}`" class="accordion-collapse collapse">
        <div class="accordion-body">
          <EditForvaltningsberattelseFlerarsoversikt v-if="group.xmlName === 'se-gen-base:Flerarsoversikt'"
            :arsredovisning="arsredovisning" :group-taxonomy-item="availableTaxonomyItems.childrenFlat.find(
              (item) => item.xmlName === 'se-gen-base:Flerarsoversikt',
            )!
              " :taxonomy-manager="taxonomyManager" />
          <EditForvaltningsberattelseForandringar v-else-if="group.xmlName === 'se-gen-base:ForandringEgetKapital'"
            :arsredovisning="arsredovisning" :group-taxonomy-item="availableTaxonomyItems.childrenFlat.find(
          ...
          <table v-else>
            <thead>
              <tr>
                <th scope="col">
                  <BaseEditBeloppradTitle :belopprad="createBelopprad(group)" :taxonomy-manager="taxonomyManager" />
                </th>
              </tr>
            </thead>
            <tbody>
              <EditBelopprad v-for="(belopprad, index) in groupedBelopprader[groupIndex]"
                ...
                v-model:belopprader="groupedBelopprader[groupIndex]" :comparable-num-previous-years="0"
                :string-minimum-level="1" :taxonomy-manager="taxonomyManager" string-multiline />
            </tbody>
          </table>
        </div>
      </div>
