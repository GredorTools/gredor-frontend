  ...
  <table>
    <thead>
      <tr>
        ...
        <th v-for="previousYearIndex in arsredovisning.verksamhetsarTidigare
          .length" :key="previousYearIndex" class="value-container" scope="col">
          {{
            formatDateForFlerarsoversikt(
    ...
    <tbody>
      <EditBelopprad v-for="(belopprad, index) in belopprader" :key="belopprad.taxonomyItemName"
        v-model:belopprad="belopprader[index]" v-model:belopprader="belopprader" :comparable-num-previous-years="Math.min(maxNumPreviousYears, 3)
          " :string-minimum-level="1" :taxonomy-manager="taxonomyManager" />
    </tbody>
  </table>
</template>
