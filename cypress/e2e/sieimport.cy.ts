...
describe("importing SIE files", () => {
  ...
  describe("1 räkenskapsår", () => {
    ...
    it("generates correct content on page", () => {
      startNewArsredovisningWithValidSIETestFile("SIETest1Rakenskapsar.se");
      ...
      cy.get(
        "#arsredovisning-for-export table.forandringar-table tr:nth-child(1) td:nth-child(2)",
      ).should("have.text", "–");
      cy.get(
        "#arsredovisning-for-export table.forandringar-table tr:nth-child(1) td:nth-child(3)",
      ).should("have.text", "–");
      cy.get(
      ...
      cy.get(
        '#arsredovisning-for-export tr:nth-child(2) [name="se-gen-base:Aktiekapital"]',
      ).should("have.text", "25 000");
      cy.get(
        '#arsredovisning-for-export tr:nth-child(2) td:nth-child(3) [name="se-gen-base:BalanseratResultat"]',
      ).should("have.text", "100 229");
      cy.get(
        '#arsredovisning-for-export tr:nth-child(2) [name="se-gen-base:AretsResultatEgetKapital"]',
      ).should("have.text", "48 867");
      cy.get(
        '#arsredovisning-for-export tr:nth-child(2) [name="se-gen-base:ForandringEgetKapitalTotalt"]',
  ...
  describe("2 räkenskaprår", () => {
    ...
    it("generates correct content on page", () => {
      startNewArsredovisningWithValidSIETestFile("SIETest2Rakenskapsar.se");
    ...
    it("shows correct warnings for file with unmapped accounts", () => {
      ...
      cy.get('[data-testid="new-arsredovisning-sie-file-input"]').selectFile(
        `cypress/fixtures/input/sie/SIETest2RakenskapsarWithUnmappedAccounts.se`,
        {
          action: "drag-drop",
        },
      );
      ...
      cy.get("div.message-modal-content p:nth-child(2)").should(
        "have.text",
        "Varning: Konto 4990 kunde inte mappas till någon post i årsredovisningen. Detta beror vanligtvis, men inte alltid, på att kontoplanen i din bokföring inte följer BAS fullt ut. Du behöver troligtvis lägga in värdet på kontot (3060,76 aktuellt räkenskapsår) i årsredovisningen manuellt.",
      );
      cy.get("div.message-modal-content p:nth-child(3)").should(
        "have.text",
        'Varning: Summaraden "Resultat efter finansiella poster" har antingen avrundningsfel eller ett felaktigt värde p.g.a. saknad kontomappning (se varningarna längre upp). Det är möjligt att du behöver justera de belopp som summan bygger på.',
      );
      cy.get("div.message-modal-content p:nth-child(4)").should(
        "have.text",
        'Varning: Summaraden "Årets resultat" har antingen avrundningsfel eller ett felaktigt värde p.g.a. saknad kontomappning (se varningarna längre upp). Det är möjligt att du behöver justera de belopp som summan bygger på.',
      );
    });
...
function startNewArsredovisningWithValidSIETestFile(
  fileName: "SIETest1Rakenskapsar.se" | "SIETest2Rakenskapsar.se",
) {
  // Börja på ny årsredovisning
  cy.get("#newArsredovisningBtn").click();
  cy.wait(1000);
  cy.get('[data-testid="new-arsredovisning-sie-file-input"]').selectFile(
    `cypress/fixtures/input/sie/${fileName}`,
    {
      action: "drag-drop",
    },
  );
